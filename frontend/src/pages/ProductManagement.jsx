...
const ProductManagement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let imageUrl = formData.image; // Retain existing image if not changed

            if (formData.image && formData.image instanceof File) {
                const uploadResponse = await uploadToCloudinary(formData.image);
            ...
            const productData = {
                ...
                image:
                    imageUrl ||
                    "https://www.hmmawards.com/wp-content/uploads/woocommerce-placeholder-300x300.png",
            };
            ...
            setProducts(
                editId
                    ? products.map((p) => (p._id === editId ? product : p))
                    : [...products, product]
            );
            setEditId(null);
            setFormData({
    ...
    const handleEdit = (id) => {
        ...
        setFormData({
            ...productToEdit,
            image: productToEdit.image, // Retain existing image URL
        });
        setEditId(id);
        setFormVisible(true);
    };
        ...
        <div className="flex bg-white font-kulim">
            ...
            <div className="flex-1 flex flex-col">
                ...
                <div className="p-6 flex-1">
                        ...
                        <div className="bg-gray-100 shadow-xl rounded-lg p-8 mb-8 max-w-lg mx-auto">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <label className="block w-full border-2 border-dashed p-6 text-center cursor-pointer rounded-md">
                                    ...
                                    {formData.image && (
                                        <p className="text-green-900 mt-2">{formData.image.name}</p>
                                    )}
                                </label>

                                <button
                                    type="submit"
                                    className="w-full bg-green-900 text-white py-3 rounded-md text-lg font-semibold"
                                >
                    ...
                    <div className="mt-8">
                        <h3 className="text-3xl font-bold mt-12 mb-4 text-center">
                            Product List
                        </h3>
                        {products.length > 0 ? (
